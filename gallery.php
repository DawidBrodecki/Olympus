...
<div id="container">
	...
	?>
		...
		<div id="modalBox_user_photo" class="modal">
		  <div class="modal-content">
			<div class="modal-header" >
			  <span class="closeBox_user_photo">&times;</span>
			  <form enctype="multipart/form-data" action="php/gallery_upload.php" method="post" >
			  <div style="font-size:2.4rem; font-weight:550; font-family:Segoe UI; padding:10px; color:#e3e3e3; display:inline-block; ">Dodaj zdjęcie do galerii:</div>
			  	<div style="display:inline; " ><label for="photo_upload" class="custom-file-upload photo_file_upload"><i class="fas fa-upload" style="font-size:1.5rem; color:#e3e3e3;"></i>&nbsp;&nbsp;Dodaj zdjęcie</label>
							<input style="" id="photo_upload" type="file" name="photo_upload" onchange="loadFile2(event)">
						</div>
					<div style="clear:both;"></div>
			</div>
			<div class="modal-body" style=" text-align:center;">
		...
		<div id="upperbar">
			...
			<div id="Mainbar">
				...
				<div id="NavOptions">
					<div class="NavOptions" style="margin-bottom:2rem;">Moja galeria:</div> 
					<div class="NavOptions" style="float:right;  margin-top:-2px; "><span class="underline" style="font-size:16px ;color:#0f96f0; margin-right:5px; cursor:pointer;"><button id="Btn_user_photo" style="font-size:16px ;color:#0f96f0; cursor:pointer;" class="ButtonGlobal underline">Dodaj zdjęcie</button></span></div> 	
					</div>
					<div style="clear:both;"></div>		
					<div style="float:left; padding-bottom:1rem; padding-left:;">
					<?php
						...
						$current_user_id = $_SESSION['id_user'];
						$sql = "SELECT photo FROM gallery WHERE id_user=$current_user_id";
						...
						{			
						echo
							'<div style="float:left; padding-bottom:1rem; padding-left:1%;">'.
							'<table style="">'.
								'<tr >'.
									'<td class="padding">'.
										'<div style="position:relative;"><a class="example-image-link" href="data:image/jpeg;base64,'.base64_encode($row['photo']).'" data-lightbox="example-set" data-title="&nbsp;"><img  src="data:image/jpeg;base64,'.base64_encode($row['photo']).'"  height="155px" width="155px" style="border-radius: 12px 12px 12px 12px; "></a>'.
											'<div style="position:absolute; right:.8rem; top:.8rem;"><div class="tooltip_global"><button class="trash_delete_photo_icon ButtonGlobal" type="submit" name=""><i class="far fa-trash-alt"></i></button><span class="tooltiptext_global">Usuń</span></div></div>
										</div>'.
									'</td>'. 
								'</tr>'.
							'</table>'.
